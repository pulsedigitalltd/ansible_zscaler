...
validate_inventory() {
    ...
    if [[ -f "inventory/hosts" ]]; then
        ...
        if ansible-inventory -i inventory/hosts --list >/dev/null 2>&1; then
            local host_count=$(ansible-inventory -i inventory/hosts --list | jq '.["_meta"]["hostvars"] | length' 2>/dev/null || echo "0")
...
[linux_clients]
# Uncomment and configure your Linux systems
# Replace IP addresses and usernames with your actual values
# linux-host-01 ansible_host=192.168.1.100 ansible_user=admin
# linux-host-02 ansible_host=192.168.1.101 ansible_user=admin

[macos_clients] 
# Uncomment and configure your macOS systems  
# Replace IP addresses and usernames with your actual values
# macos-host-01 ansible_host=192.168.1.200 ansible_user=admin
# macos-host-02 ansible_host=192.168.1.201 ansible_user=admin
...
EOF
        ...
        print_status "warn" "Edit inventory/hosts and uncomment/configure your hosts"
    fi
...
run_connectivity_test() {
    ...
    # Check if we have any active hosts first
...
EOF
    ...
    local host_count=$(ANSIBLE_CONFIG="$temp_config" ansible-inventory -i inventory/hosts --list 2>/dev/null | jq -r '._meta.hostvars | keys | length' 2>/dev/null || echo "0")
    rm -f "$temp_config"
    
    if [[ "$host_count" -eq 0 ]]; then
        print_status "warn" "No hosts configured to test connectivity"
        return 0
    fi
    
    # Test connectivity with vault password if available
    local vault_args=""
    if [[ -f ".vault_pass" ]]; then
        vault_args="--vault-password-file .vault_pass"
    fi
    
    if ansible all -i inventory/hosts $vault_args -m ping --one-line >/dev/null 2>&1; then
        print_status "ok" "All configured hosts are reachable"
        return 0
    else
        print_status "warn" "Some hosts may not be reachable"
        print_status "info" "This is expected if hosts aren't set up yet"
        print_status "info" "Run 'ansible all -i inventory/hosts -m ping $vault_args' for details"
        return 1
    fi
}
